class Public::MembersController < ApplicationController
  ...
  before_action :ensure_current_member, only: %i[edit update unsubscribe]
  # 会員の情報関連（マイページ）
  ...
  def member_params
    params.require(:member).permit(:profile_image, :first_name, :last_name, :first_name_kana, :last_name_kana,
                                   :nickname, :introduction, :email)
  end

  def ensure_current_member
    @member = Member.find(params[:id])
    redirect_to members_path, al: '権限がありません' unless @member == current_member
  end
end
